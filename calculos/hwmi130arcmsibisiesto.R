...
tasmaxhis<-tasmax[,,1:11323] # 12053 hasta 32a (2002)
# Este valor es correcto si hay bisiestos
#tasmaxfut<- ncvar_get(ncfut, "tasmax")
#tasmaxfut<- tasmax[,,36891:47847] # ultimos 30a
tasmaxfut<-tasmax
...
for (xx in 1:xmax){
for (yy in 1:ymax){
res<-hwmi(1971,tasmaxhis[xx,yy,],1971,tasmaxfut[xx,yy,])
resd<-hwmid(1971,tasmaxhis[xx,yy,],1971,tasmaxfut[xx,yy,])
#hwmitot<-res[[xx]][[yy]]$hwmi
hwmitot<-res$hwmi
hwmidtot<-resd$hwmi
hwmiindex<-hwmitot[1:atot,1]
hwmilen<-hwmitot[1:atot,2]
hwmid<-hwmidtot[1:atot,1]
hwmimatrixi[xx,yy,1:atot]<-hwmiindex
hwmimatrixl[xx,yy,1:atot]<-hwmilen
hwmidmatrixi[xx,yy,1:atot]<-hwmid
